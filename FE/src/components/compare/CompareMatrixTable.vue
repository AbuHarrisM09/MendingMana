<template>
  <div class="w-full bg-white/70 backdrop-blur-xl border border-white shadow-xl shadow-slate-100/50 rounded-3xl overflow-hidden animate-fade-in">
            ...
            <div class="space-y-1">
              <template v-for="group in comparedGadgetsData.specGroups" :key="'m-' + group.group + '-' + gIndex">
                ...
                <div 
                  ...
                  class="grid grid-cols-5 gap-3 px-4 py-2.5 border-b border-slate-50 last:border-0 text-[11px]"
                >
                  <span class="font-bold text-slate-500 col-span-2">{{ spec.key }}</span>
                  <span v-if="spec.values[gadget.id] !== null" class="text-slate-800 font-medium text-right col-span-3 break-all sm:break-words">
                    {{ spec.values[gadget.id] }}
                  </span>
                  <span v-else class="text-slate-300 italic text-right col-span-3">—</span>
                </div>
              </template>
            </div>
          </div>
