        ...
        <div class="h-16 flex items-center justify-between">
          ...
          <div class="hidden md:flex flex-1 max-w-md mx-8 relative group">
            ...
            <div
              ...
              class="absolute top-12 left-0 right-0 bg-white/95 backdrop-blur-xl border border-slate-100 rounded-2xl shadow-2xl py-2 z-50 overflow-hidden"
            >
              <div class="px-3.5 py-1.5 border-b border-slate-100 text-xs font-semibold text-slate-400 tracking-wider">
                Saran Gadget
              </div>
              <router-link
                v-for="gadget in searchSuggestions"
                :key="gadget.id"
                :to="'/gadget/' + gadget.id"
                class="flex items-center gap-3 px-3.5 py-2.5 hover:bg-slate-50 transition-colors cursor-pointer"
              >
                <!-- Thumbnail -->
                <img
                  ...
                  :alt="gadget.name"
                  class="w-9 h-9 object-contain rounded-lg bg-slate-100 border border-slate-50 p-0.5 shrink-0"
                />
                
                <!-- Info -->
                <div class="flex-1 min-w-0">
                  <div class="text-sm font-bold text-slate-800 truncate">{{ gadget.name }}</div>
                  <div class="text-xs text-slate-500 capitalize">{{ gadget.brand }} &bull; {{ gadget.category }}</div>
                </div>
                ...
                <div class="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded-lg shrink-0">
                  ...
                  <span class="text-xs font-bold text-amber-700">
                    {{ gadget.averageRating ? gadget.averageRating.toFixed(1) : '5.0' }}
                  </span>
                </div>
              </router-link>
            </div>
...
import {
  Search, ChevronDown, LogOut, Heart, Shield, LayoutDashboard, X, Star
...
const searchSuggestions = computed(() => {
  ...
  return gadgets.value
    .filter(g => g.name.toLowerCase().includes(query) || (g.brand && g.brand.toLowerCase().includes(query)))
    .slice(0, 5)
})
